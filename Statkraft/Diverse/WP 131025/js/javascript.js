...
function animateProgressBar(totalValue)
{
	...
	{		
		...
		var i = setInterval(function ()
	    {
			...
			{
				clearInterval(i);
				$("#totalProductionValueMeter .value").text(totalValue < 1 ? totalValue.toPrecision(2) : totalValue);
			}
			else
			{
...
function refresh()
{
	...
	var source = "http://nooslpev01t/Statkraft.Ts.Information/WMSLive.svc/GetWindParkSummaryNow";
	if (window.location.hash === "#devmode") {
		source = "http://dahlgren.tv/johan/jobb/statkraft/jsonData2.php";
	}
	...
	var onSuccess = function(jsonResponse) {
		...
		response.canSupply = getCanSupply(jsonResponse.HousholdSupplies);
