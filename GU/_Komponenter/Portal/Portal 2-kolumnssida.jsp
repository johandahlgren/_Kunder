...
<div class="col25">
	<div id="menuComp">
		<!--eri-no-index-->
		<ig:slot id="leftColumn" inherit="true" allowedComponentGroupNames="PortalLeftColumn"></ig:slot>
		<!--/eri-no-index-->
	</div>
</div>
		...
		<c:when test="${param.feedbackForm == 'true'}">
			...
			<c:choose>
				<c:when test="${empty decryptedSubject}">
					<common:encrypt id="subject" value="Kontaktformul&auml;r"/>
				</c:when>
				<c:otherwise>
				...
				<c:otherwise>
					<c:choose>
						...
						<c:otherwise>
							...
							<p>Meddelandet kunde inte skickas - var god kontakta oss per telefon (031-773 1000) och p&aring;tala felet. Felet som rapporterades var: <c:out value="${commonMailTagException.message}"/></p>
						</c:otherwise>
					</c:choose>
				</c:otherwise>
			</c:choose>
